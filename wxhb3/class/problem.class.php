...
class problemClass extends database_wcc {
	...
	function getCount3($username){
		$sql = "select count(*) as number from problem where username2='$username' ";
    	return $this->getTotalNumber($sql);
	}	
	function getAll(){
	...
	function getAll3($username){
		$sql = "select * from problem where username2='$username' order by id desc";
		return $this->selectArray($sql);		
	}	
	...
	function add2($pid,$oid,$title,$imgfile,$memo,$zdy,$username1,$username2){		
		$sql = "INSERT  INTO problem(pid,oid,title,imgfile,memo,zdy,username1,username2,createTime) Values('$pid','$oid','$title','$imgfile','$memo','$zdy','$username1','$username2',now())";
    	//echo $sql;
    	//echo '<br>';
    	$this->update_sql($sql);
	}	
