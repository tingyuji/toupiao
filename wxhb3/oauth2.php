...
class Wxapi {
     ...
     */
    public function pay($openid1,$openid2,$fee,$orderdate,$id,$db=null)
    {
        ...
        $wxHongBaoHelper->setParameter("nick_name", '赚米吧');//提供方名称
        $wxHongBaoHelper->setParameter("send_name", '赚米吧');//红包发送者名称
        $wxHongBaoHelper->setParameter("re_openid", $openid1);//相对于医脉互通的openid
        $wxHongBaoHelper->setParameter("total_amount", 100*$fee);//付款金额，单位分
        $wxHongBaoHelper->setParameter("min_value", 100);//最小红包金额，单位分
        $wxHongBaoHelper->setParameter("max_value", 100);//最大红包金额，单位分
        $wxHongBaoHelper->setParameter("total_num", 1);//红包収放总人数
        $wxHongBaoHelper->setParameter("wishing", '不满九个累计佣金');//红包祝福诧
        $wxHongBaoHelper->setParameter("client_ip", '59.56.110.142');//调用接口的机器 Ip 地址
        ...
        if($result_code=='SUCCESS'){
            ...
            $dailyitemClass->update($openid2,1);

            // $feeClass=new feeClass();
            // $feeClass->update3($openid2,1);            
