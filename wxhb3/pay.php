...
class Packet{
	...
	private function userinfo($param){ 
		...
		if($get=='access_token' && !empty($code)){
			...
			$this->wxapi->get_authorize_url('http://www.fangdan8.com/wxhb/index.php?param=access_token','STATE');
		}
	}
    /**
     * 微信红包
	...
	private function wxpacket($param){
		return $this->wxapi->pay($param['openid1'],$param['openid2'],$param['fee'],$param['orderdate'],$param['id']);
	}
}
?>