...
class wechatCallbackapiTest{
    ...
    $doneClass=new doneClass();
		...
		if (!empty($postStr)){
                ...
                $newsTpl2="<xml>
                          ...
                          <Articles>
                          <item>
                          <Title><![CDATA[我的日程列表]]></Title>
                          <Description><![CDATA[可邀请好友参与]]></Description>
                          <PicUrl><![CDATA[%s]]></PicUrl>
                          <Url><![CDATA[%s]]></Url>
                          </item>
                          </Articles>
                   ...
                   case "event":
                          switch (strtolower($postObj->Event)){
                            ...
                            case "click":
                                    ...
                                    switch ($key) {
                                        case 'key1':
                                            $total=$mappingClass->getCount2($toUsername);
                                            //$total=0;
                                            if($total==0){

                                              $mappingClass->add($toUsername);
                                              $contentStr="您需要小萌的账户密码登录后才可以使用"."\n";
                                              $contentStr.="请输入您的账号："."\n";
                                              $contentStr.="(如果您没有账号，请点击菜单【我的-账号注册】注册个账号吧)";
                                              $mappingClass->update3($toUsername,1);
                                            ...
                                            if($total!=0){
                                              ...
                                              $items=$mappingClass->getAll2($toUsername);
                                              $item=$items[0];
                                              $status=$item['status'];
                                              $step=$item['step'];
    ...
    如果无法完成该任务或者任务目标网页出错，可以回复【放弃】跳过该任务。';
                                                  ...
                                                  $price2=$item['price2'];
                                                  $img1=$item['img1'];
                                                  $imgUrl='http://www.xiaomutong.com.cn/toupiao/'.$img1;
                                                ...
                                                if($total==0){
                                                  ...
                                                  $total=$taskClass->getCount3($toUsername);
                                                  if($total==0){
                                        ...
                                        case 'key3':
                                            ...
                                            $url='http://xiaomutong.com.cn/wx5/key3.php';
                                            $url2='https://open.weixin.qq.com/connect/oauth2/authorize?appid=wxc3e3f3ee1d734a11&redirect_uri='.$url.'&response_type=code&scope=snsapi_userinfo&state=STATE#wechat_redirect';

                                            $PicUrl ='http://xiaomutong.com.cn/wx5/003.png';
                                            $contentStr.=$url2;
                                            $msgType2 = "news";
                                            $resultStr = sprintf($newsTpl3, $fromUsername, $toUsername, $time, $msgType2, $PicUrl,$contentStr);
                                            break;
                              ...
                              if($total!=0){
                                ...
                                if($step==2){
                                  ...
                                  if($total!=0){
                                    ...
                                    $type='已完成';
                                    $doneClass->add($toUsername,$pid,$title,$type);
                                ...
                                if($total!=0){
                                  ...
                                  if($step==2){
                                    ...
                                    if($total!=0){
                                      ...
                                      $abandonClass->add($toUsername,$pid,$title);
                                      $type='已放弃';
                                      $doneClass->add($toUsername,$pid,$title,$type);
                                ...
                                default:
                                  ...
                                  $step=$item['step'];
                                  if($step==1){
                                        $mappingClass->update($toUsername,$keyword);

                                        $contentStr="请输入账户[".$keyword."]对应的密码：";
                                        $mappingClass->update3($toUsername,2);
                                  ...
                                  if($step==2){
                                        $mappingClass->update2($toUsername,$keyword);
                                        ...
                                        if($total!=0){
                                          $status=='验证成功';
                                          $mappingClass->update4($toUsername,'验证成功');
                                          $mappingClass->update3($toUsername,2);
                                          $contentStr="登录成功，您现在可以使用任务小萌的功能了。";
                                        }
                                        if($total==0){
                                          $status=='验证失败';
                                          $mappingClass->update4($toUsername,'验证失败');
                                          $mappingClass->update3($toUsername,1);
                                          $contentStr="登录失败，请重新输入账号:";
