...
class ecodeClass extends database_wcc {
	...
	function getCountByKeyword($keyword){
		$sql = "select count(*) as number from ecode where ecode like '%$keyword%' ";
    	return $this->getTotalNumber($sql);
	}	

	function getCountByecode($ecode){
		$sql = "select count(*) as number from ecode where ecode = '$ecode' ";
    	return $this->getTotalNumber($sql);
	}	

	function getCountByecode2($ecode){
		$sql = "select count(*) as number from ecode where ecode = '$ecode' and status='未用'";
    	return $this->getTotalNumber($sql);
	}		

	function getAllItems($offset,$rows){
		$sql = "select * from ecode order by id asc limit $offset,$rows";
